...
**Reproducibility.** Supplementary Material S1 and `paper/CODE_AVAILABILITY.md` list environment pins, frozen checkpoint aliases, and a one-command artifact regeneration path. A public repository URL and commit hash will be inserted at acceptance (or provided on request under data-use constraints for TUH/CAUEEG).
