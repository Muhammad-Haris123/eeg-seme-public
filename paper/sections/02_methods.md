...
No paired pre/post-dose EEG is available. Direction measures concordance with the **training constrained signature**, a prior-shaped model-derived reference. Circularity is intentional and bounded by prior-only (mean \(r=0.636\), 10/10), matched unconstrained CVAE (\(\alpha_c=0\)), and a ridge imitation control on locked CVAE-simulated targets (not observed post-dose EEG).

**SEME primary versus secondary endpoints.**
- **Primary (S):** block-level signed concordance of cohort-mean drug-minus-baseline vectors versus the training constrained signature, with constrained versus unconstrained as the main contrast.
- **Secondary / sensitivity (E, continuous \(r\)):** Pearson effect-vector \(r\) on the same vectors, including fold and train-seed sensitivity. Locked high \(r\) is reported as a checkpoint-specific illustration, not as the primary success claim.
- **Reporting axes (M; Encoding):** magnitude \(\bar{s}\) as a diagnostic discriminator across the escalation sequence, and residual label information in \(\mu_{\mathrm{base}}\) versus theta/alpha and 2185-D feature references.

Supporting analyses: exploratory \(\alpha_c\) sweep; paired DeLong tests on stored OOF probe scores (\(n=727\)) [DeLong1988]; matched ChemBERTa-versus-padded-one-hot ablation (non-locked checkpoints). The pre-specified primary contrast for signed concordance is constrained versus matched unconstrained twins on TUH, OSF, and P-ADIC. Uncertainty for \(r\) uses subject-bootstrap percentile intervals (B = 2000). Magnitude uses Mann-Whitney and Cohen's d. Probe permutations use \(N_{\mathrm{perm}}=200\). No multiplicity correction was applied across the escalation sequence or DeLong pairs. Full definitions are in S1.
