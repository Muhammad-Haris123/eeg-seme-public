"""SHA256 integrity for domain-shift run. Writes only under domain_shift/."""
...
def collect_protected(root: Path) -> List[Path]:
    ...
    ens = root / "models" / "validation" / "fold_ensemble"
    for p in sorted(ens.rglob("*")):
        if p.is_file() and "domain_shift" not in p.parts:
            # include ensemble + ensemble bootstrap artifacts
            files.append(p)
...
def write_report(report: Path, pre_p: Path, post_p: Path, pre: Dict, post: Dict) -> bool:
    ...
    lines = [
        "# Integrity report (domain-shift run)",
        f"# timestamp: {datetime.now().isoformat()}",
...
def main() -> int:
    if len(sys.argv) < 2 or sys.argv[1] not in {"pre", "post", "compare"}:
        print("Usage: domain_shift_integrity.py {pre|post|compare}", file=sys.stderr)
        return 2
    root = _root()
    out = root / "models" / "validation" / "domain_shift"
    ...
    if mode in {"pre", "post"}:
        ...
        print(f"[wrote] {out / name} ({len(mapping)})")
        return 0
